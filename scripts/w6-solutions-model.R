...
cat("model{
  # Priors
  beta_0 ~ dnorm(0, 0.1^-2) # I changed the sd from 2 to 0.1
...
# Note that the above model contains the observation-level random effect used to
...
# Solution: I altered my prior for `beta_0`, changing the sd from our standard
# of 2 to 0.1. Run the `w6-solutions-model.R` and `w6-solutions-sensitivity.R`
# scripts to see what I found. Note that there was still considerable overlap in 
# the 95% CIs, but this was still quite a strong prior! (The data-generating 
# value of the intercept parameter for the starfish data was 1.0 and my model
# with the strong prior is estimating a median of 0.06). This suggests that it 
# is also important to look at the differences in the parameter estimates and 
# 95% CIs, and to see if the 95% CIs are pressing up against the edges of the 
# density of the prior distribution you've assigned that parameter.
